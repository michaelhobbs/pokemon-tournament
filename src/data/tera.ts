...
const teamTypeCounts = (team: string[]): Record<TypeName, number> => {
  const counts = Object.fromEntries(TYPES.map((type) => [type, 0])) as Record<
    TypeName,
    number
  >;
...
const weaknessMap = (team: string[]): Record<TypeName, number> => {
  const map = Object.fromEntries(TYPES.map((type) => [type, 0])) as Record<
    TypeName,
    number
  >;
  for (const name of team) {
 ...
 */
const threatScore = (
  map: Record<TypeName, number>,
  oppCounts: Record<TypeName, number>,
): number => TYPES.reduce((sum, type) => sum + map[type] * oppCounts[type], 0);
 ...
 */
export function analyzeTeraDefensive(
  team: string[],
  opponent: string[],
): TeraDefensiveSuggestion[] {
  ...
  for (const teraType of TYPES) {
    ...
    for (const carrier of team) {
      ...
      if (!best || score < best.score) {
        const fixed = TYPES.filter(
          (type) => oppCounts[type] > 0 && base[type] > map[type],
 ...
 */
export function analyzeTeraOffensive(
  team: string[],
  opponent: string[],
): TeraOffensiveSuggestion[] {
