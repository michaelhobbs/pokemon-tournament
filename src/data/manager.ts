...
export function actionLabel(action: Action): string {
	...
	if (action.kind === 'travel') return `TRAVELLING TO ${player?.hometown ?? '???'}`;
...
export function loadState(): GameState {
	try {
		...
		if (!parsed || typeof parsed !== 'object' || (parsed as GameState).version !== 1) {
			return defaultState();
		}
		const state: GameState = {
			version: 1,
			...
			items: {
				'rare-candy': typeof parsed.items?.['rare-candy'] === 'number' ? parsed.items['rare-candy'] : 0,
				'max-repel': typeof parsed.items?.['max-repel'] === 'number' ? parsed.items['max-repel'] : 0,
			},
...
export function travelDurationMs(playerNumber: number): number {
	...
	const distance = Math.abs(marker.col - HOME_BASE.col) + Math.abs(marker.row - HOME_BASE.row);
	return Math.min(TRAVEL_CAP_MS, TRAVEL_BASE_MS + distance * TRAVEL_PER_UNIT_MS);
}
...
export function startAction(
	...
	if (kind === 'train') {
		humon.action = { kind, startedAt: Date.now(), durationMs: TRAIN_MS, seed: newSeed() };
	...
	if (kind === 'travel') {
		...
		humon.action = { kind, startedAt: Date.now(), durationMs: travelDurationMs(target), seed: newSeed(), target };
	...
	if (kind === 'gym') {
		...
		if (humon.team.length === 0) return { ok: false, error: `${humonName(humon)} NEEDS A TEAM FIRST` };
		const rec = recommendedLevel(target);
		if (humon.level < rec) return { ok: false, error: `GYM ${player.number} REQUIRES LV ${rec}` };
		humon.action = { kind, startedAt: Date.now(), durationMs: GYM_MS, seed: newSeed(), target };
...
export async function startGymAction(
	...
	humonId: string,
	bossNumber: number,
): Promise<{ ok: true; win: boolean; log: string[]; turns: BattleTurn[] } | { ok: false; error: string }> {
	const humon = humonById(state, humonId);
	if (!humon) return { ok: false, error: 'HUMON NOT FOUND' };
	if (humon.action) return { ok: false, error: `${humonName(humon)} IS BUSY` };
...
function resolveTrain(state: GameState, humon: Humon, action: Action): void {
	const rand = mulberry32(hashString(`${humon.id}:train:${action.startedAt}:${action.seed}`));
	humon.xp += TRAIN_XP;
...
function resolveTravel(state: GameState, humon: Humon, action: Action): void {
	...
	const rand = mulberry32(hashString(`${humon.id}:travel:${action.startedAt}:${action.seed}`));
	humon.xp += TRAVEL_XP;
	humon.level = levelFor(humon.xp);
	const pay = randInt(rand, CURRENCY.travel.min, CURRENCY.travel.max);
	state.currency += pay;
	log(state, `${humonName(humon)} RETURNS FROM ${townName}. +${TRAVEL_XP} XP +$${pay}`);
	...
	const roll = pool.length > 0 && (guaranteed || rand() < catchChance(humon.level));
	if (!roll) {
	...
	const pick = unowned.length > 0 ? unowned[Math.floor(rand() * unowned.length)] : pool[Math.floor(rand() * pool.length)];
...
function resolveGym(state: GameState, humon: Humon, action: Action): void {
	...
	const rand = mulberry32(hashString(`${humon.id}:gym:${action.startedAt}:${action.seed}`));
...
export function unlockSecret(state: GameState, key: SecretHumonKey): { ok: true } | { ok: false; error: string } {
	const spec = SECRET_HUMONS[key];
	if (state.unlocked.includes(key)) return { ok: false, error: `${spec.name} IS ALREADY IN THE SQUAD` };
	if (!state.visited.includes(spec.page)) return { ok: false, error: `FIND PAGE ${spec.page} FIRST` };
	if (state.currency < spec.cost) return { ok: false, error: `NOT ENOUGH CURRENCY (NEED $${spec.cost})` };
...
export function useRareCandy(state: GameState, humonId: string): { ok: true } | { ok: false; error: string } {
	const humon = humonById(state, humonId);
	if (!humon) return { ok: false, error: 'HUMON NOT FOUND' };
	if (state.items['rare-candy'] <= 0) return { ok: false, error: 'NO RARE CANDIES' };
	state.items['rare-candy'] -= 1;
