...
import { WEEKS, FIRST_HALF_LAST_WEEK, FIRST_HALF_WEEKS, SECOND_HALF_WEEKS, weekFor } from './matches';
...
describe('WEEKS', () => {
	...
	it('first 9 weeks have results, remaining are TBD', () => {
		const firstHalf = WEEKS.filter((w) => w.number <= FIRST_HALF_LAST_WEEK);
		const secondHalf = WEEKS.filter((w) => w.number > FIRST_HALF_LAST_WEEK);
		for (const week of firstHalf) {
			for (const match of week.matches) {
				expect(match.wins1).toBeDefined();
				expect(match.wins2).toBeDefined();
			}
		}
		for (const week of secondHalf) {
			for (const match of week.matches) {
				expect(match.wins1).toBeUndefined();
				expect(match.wins2).toBeUndefined();
			}
		}
	});
});
...
describe('FIRST_HALF_WEEKS / SECOND_HALF_WEEKS', () => {
	...
	it('all weeks accounted for', () => {
		expect(FIRST_HALF_WEEKS.length + SECOND_HALF_WEEKS.length).toBe(WEEKS.length);
	});
});
