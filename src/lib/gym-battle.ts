...
export type GymPrompt =
  ...
  | {
      kind: "switch";
      slots: { index: number; must: boolean; mon: GymMonView | null }[];
    };
...
export function moveEffectiveness(
  ...
  const defTypes = (
    target.types ??
    POKEMON_TYPES[target.name] ??
    dexSpeciesTypes(target.species || target.name)
  ).map((t) => t.toLowerCase());
...
function parseCondition(condition: string): {
  ...
  const status =
    !fainted && parts.length > 1
      ? (parts.slice(1).find((p) => STATUS_IDS.includes(p)) ?? null)
      : null;
...
export function targetOptionsFor(
  ...
  switch (target) {
    case "normal":
    case "adjacentFoe":
      return livingFoes.map((m) => ({
        loc: m.activeSlot === 1 ? 2 : 1,
        label: m.name,
        effect: effectOn(m),
      }));
    case "adjacentAlly":
      return ally && !ally.fainted
        ? [{ loc: -(allyIndex + 1), label: ally.name, effect: effectOn(ally) }]
        : [];
    case "adjacentAllyOrSelf": {
      ...
      if (self)
        options.push({
          ...
          effect: effectOn(self),
        });
      if (ally && !ally.fainted)
        options.push({
          loc: -(allyIndex + 1),
          label: `${ally.name} (ALLY)`,
          effect: effectOn(ally),
        });
      return options;
    }
    case "any": {
...
export class GymBattle {
  ...
  private lastActiveIdents: Record<"p1" | "p2", (string | null)[]> = {
    p1: [null, null],
    p2: [null, null],
  };
  ...
  private sideConditions: Record<"p1" | "p2", Set<string>> = {
    p1: new Set(),
    p2: new Set(),
  };
  private sideStarts: Record<"p1" | "p2", Map<string, number>> = {
    p1: new Map(),
    p2: new Map(),
  };
  private turn = 0;
    ...
    switch (cmd) {
      ...
      case "replace": {
        ...
        if (cmd === "replace" && !condition) {
          ...
          const prevMon =
            prevIdent && prevIdent !== ident
              ? this.members.get(prevIdent)
              : undefined;
        ...
        if (cmd === "drag") this.pushLog(`${mon.name} was dragged out!`);
          ...
          this.pushLog(`${mon.name} revealed itself!`);
      ...
      case "move": {
        ...
        this.pushLog(
          target
            ? `${attacker.name} used ${move} on ${target.name}!`
      ...
      case "-damage": {
        ...
        if (!mon) break;
        const prevPct =
          mon.maxHp > 0 ? Math.round((mon.hp / mon.maxHp) * 100) : 0;
        ...
        if (reason && source)
          this.pushLog(
            `${mon.name} took ${lost}% damage from ${reason} (${source})!`,
          );
        else if (reason)
          this.pushLog(`${mon.name} took ${lost}% damage from ${reason}!`);
      ...
      case "-heal": {
        ...
        const source = ofValue ? (this.byIdent(ofValue)?.name ?? null) : null;
        if (reason === "DRAIN" && source)
          this.pushLog(
            `${mon.name} siphoned ${gained}% health from ${source}!`,
          );
        else if (reason && source)
          this.pushLog(
            `${mon.name} regained ${gained}% health from ${reason} (${source})!`,
          );
        else if (reason)
          this.pushLog(
            `${mon.name} regained ${gained}% health from ${reason}!`,
          );
      ...
      case "-unboost": {
        ...
        this.pushLog(
          cmd === "-boost"
            ? `${mon.name}'s ${stat} rose${degree}!`
            : `${mon.name}'s ${stat} fell${degree}!`,
        );
        break;
      }
      case "-setboost": {
      ...
      case "-clearnegativeboost": {
        ...
        if (mon) {
          for (const stat of Object.keys(mon.stages)) {
            ...
            if ((mon.stages[stat] ?? 0) > 0 === positive)
              delete mon.stages[stat];
          }
        }
        break;
      }
      case "-invertboost": {
        ...
        if (mon)
          for (const stat of Object.keys(mon.stages))
            mon.stages[stat] = -(mon.stages[stat] ?? 0);
        break;
      }
      case "-clearallboost":
      ...
      case "-activate": {
        ...
        if (mon && /^ability:/i.test(rawEffect))
          mon.ability = rawEffect.replace(/^ability:\s*/i, "");
        break;
      }
      case "-item": {
        const mon = this.byIdent(parts[2] ?? "");
      ...
      case "-ability": {
        ...
        const args = parts
          .slice(3)
          .filter((arg) => !arg.startsWith("[") && arg !== "boost");
      ...
      case "cant": {
        ...
        if (reason === "FLINCH") this.pushLog(`${mon.name} FLINCHED!`);
          ...
          this.pushLog(`${mon.name} is fast asleep!`);
        else if (reason === "FRZ") this.pushLog(`${mon.name} is frozen solid!`);
        else if (reason === "RECHARGE")
          this.pushLog(`${mon.name} must recharge!`);
        else if (reason === "PAR")
          this.pushLog(`${mon.name} is paralyzed! It cannot move!`);
        else {
          const move = cleanEffect(parts[4] ?? "");
          this.pushLog(
            move
              ? `${mon.name} cannot use ${move}!`
              : `${mon.name} cannot move!`,
          );
        }
        break;
      }
      ...
      case "-hitcount": {
        ...
        if (count > 0)
          this.pushLog(`HIT ${count} TIME${count === 1 ? "" : "S"}!`);
        break;
      }
      case "faint": {
        const mon = this.byIdent(parts[2] ?? "");
      ...
      case "-weather": {
        ...
        if (this.weather) {
          ...
          if (!upkeep || this.weatherStart === null)
            this.weatherStart = this.turn;
      ...
      case "-terrain": {
        ...
        if (this.terrain) {
          if (!upkeep || this.terrainStart === null)
            this.terrainStart = this.turn;
      ...
      case "-sidestart": {
        const side = ((parts[2] ?? "").startsWith("p1") ? "p1" : "p2") as
          "p1" | "p2";
        ...
        this.pushLog(
          `${side === "p1" ? "YOUR SIDE" : "FOE SIDE"}: ${labelSideCondition(cond)}`,
        );
        break;
      }
      case "-sideend": {
        ...
        if (this.sideConditions[side].delete(cond)) {
          ...
          this.pushLog(
            `${side === "p1" ? "YOUR SIDE" : "FOE SIDE"}: ${labelSideCondition(cond)} faded!`,
          );
        }
        break;
      }
      case "-terastallize": {
        ...
        if (mon) {
          ...
          this.pushLog(
            `${mon.name} TERASTALLIZED${type ? ` into ${type}` : ""}!`,
          );
        }
        break;
      }
      case "-start": {
        ...
        if (volatile === "typechange") {
          ...
          if (types.length > 0) {
            ...
            if (!silent)
              this.pushLog(
                `${mon.name} changed its type to ${types.join("/")}!`,
              );
          }
          ...
          !mon.volatiles.includes(volatile)
        ) {
      ...
      case "-resisted": {
        ...
        this.pushLog(
          mon
            ? `IT'S ${cmd === "-resisted" ? "NOT VERY EFFECTIVE..." : "SUPER EFFECTIVE!"} (${mon.name.toUpperCase()})`
            : cmd === "-resisted"
      ...
      case "-miss": {
        ...
        const target = targetPart ? this.byIdent(targetPart) : undefined;
        if (attacker && target)
          this.pushLog(`${attacker.name}'s attack missed ${target.name}!`);
      ...
      case "error":
        this.pushLog(
          `! ${(parts.slice(2).join("|") || "INVALID CHOICE").toUpperCase()}`,
        );
        break;
    }
    return false;
  ...
  private applyCondition(mon: GymMonView, condition: string): void {
    ...
    mon.hpPct = Math.max(
      0,
      Math.min(100, Math.round((parsed.hp / (parsed.maxHp || 1)) * 100)),
    );
    if (parsed.fainted) {
      mon.fainted = true;
      mon.status = null;
  ...
  private setActive(ident: string, letter: string): void {
    ...
    const activeIdents =
      side === "p1" ? this.p1ActiveIdents : this.p2ActiveIdents;
  ...
  private emitSnapshot(): void {
    ...
    const sideConditions = (side: "p1" | "p2"): TimedCondition[] =>
      [...this.sideConditions[side]].map((id) => {
        ...
        return {
          id,
          timer: start === undefined ? null : turnsFor(id, start, this.turn),
        };
      });
    this.onSnapshot({
      turn: this.turn,
      ...
      weatherTimer:
        this.weather && this.weatherStart !== null
          ? turnsFor(this.weather, this.weatherStart, this.turn)
          : null,
      terrainTimer:
        this.terrain && this.terrainStart !== null
          ? turnsFor(this.terrain, this.terrainStart, this.turn)
          : null,
  ...
  private emitPrompt(request: Record<string, unknown>): boolean {
    ...
    if (Array.isArray(request.forceSwitch)) {
      ...
      const slots = forceSwitch.map((must, i) => {
        const ident =
          this.p1ActiveIdents[i] ??
          this.lastActiveIdents.p1[i] ??
          `p1${i === 1 ? "b" : "a"}: ???`;
        return {
          index: i,
          must,
          mon:
            this.p1ActiveIdents[i] || this.lastActiveIdents.p1[i]
              ? (this.byIdent(ident) ?? null)
              : null,
        };
      });
    ...
    if (Array.isArray(request.active)) {
      ...
      activeData.forEach((active, i) => {
        ...
        actives.push({
          ...
          canTerastallize:
            !this.teamTeraUsed &&
            typeof active.canTerastallize === "string" &&
