...
export function importTeamText(text: string): ImportResult {
	...
	if (!trimmed) return { ok: false, error: 'NOTHING TO IMPORT - PASTE A POKEMON SHOWDOWN TEAM FIRST.' };
	let sets: ReturnType<typeof Teams.import>;
	try {
		sets = Teams.import(trimmed);
	} catch (err) {
		return { ok: false, error: `COULD NOT READ THE TEAM: ${err instanceof Error ? err.message : String(err)}`.toUpperCase() };
	}
	if (!sets || sets.length === 0) return { ok: false, error: 'NO POKEMON FOUND - CHECK THAT THE TEXT USES SHOWDOWN EXPORT FORMAT.' };
	if (sets.length !== GYM_TEAM_SIZE) {
		return { ok: false, error: `GYM BATTLES ARE 4 VS 4 - THE TEAM NEEDS EXACTLY ${GYM_TEAM_SIZE} POKEMON (FOUND ${sets.length}).` };
	}
	...
	if (problems && problems.length > 0) return { ok: false, error: `INVALID TEAM - ${problems.join(' ')}`.toUpperCase() };
	...
	const mons = sets.map((set): ImportedMon => {
		...
		const baseSpecies = dexSpecies?.exists ? String(dexSpecies.baseSpecies) : rawSpecies;
		// Teams.import back-fills name with the base forme ("Typhlosion" for
		// Typhlosion-Hisui) - that is not a nickname.
		const nickname =
			typeof set.name === 'string' && set.name && set.name !== species && set.name !== baseSpecies ? set.name : null;
		...
		return {
			...
			moves: (Array.isArray(set.moves) ? set.moves : []).map(String).filter(Boolean),
			types: dexSpecies?.exists ? dexSpecies.types.map(String) : [],
			evs: STAT_ORDER.filter(({ key }) => (evSource[key] ?? 0) > 0).map(({ key, label }) => ({ stat: key, label, value: evSource[key] ?? 0 })),
			ivs: STAT_ORDER.filter(({ key }) => ivSource[key] !== undefined && ivSource[key] !== 31).map(({ key, label }) => ({ stat: key, label, value: ivSource[key] ?? 31 })),
 ...
 */
export function monSprite(name: string): { url: string | null; loading: boolean } {
	...
	return entry.status === 'ready' ? { url: entry.url ?? null, loading: false } : { url: null, loading: entry.status === 'loading' };
}

async function fetchPokedexSprite(name: string, entry: SpriteEntry): Promise<void> {
	try {
		const res = await fetch(`https://pokeapi.co/api/v2/pokemon/${encodeURIComponent(pokedexSlug(name))}`);
		if (!res.ok) throw new Error(String(res.status));
		const data = (await res.json()) as {
			sprites?: {
				front_default?: string | null;
				versions?: { 'generation-i'?: { 'red-blue'?: { front_default?: string | null } } };
