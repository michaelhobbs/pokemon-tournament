...
function parseHp(
  hp: string,
): { hp: number; maxHp: number; pct: number } | null {
  ...
  return {
    hp: cur,
    maxHp: max,
    pct: max > 0 ? Math.round((cur / max) * 100) : 0,
  };
}

function snapshotSide(
  team: Map<string, BattleTeamMember>,
  active: string,
): BattleTeamMember[] {
...
export function parseBattleLog(
  ...
  function startTurn(num: number): void {
    ...
    current = {
      ...
      p1: [],
      p2: [],
      field: { weather: null, terrain: null, room: null },
  ...
  function resolveMember(who: string): {
    team: Map<string, BattleTeamMember>;
    member: BattleTeamMember | undefined;
  } {
  ...
  for (const chunk of raw) {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith("|poke|")) {
        ...
        if (!team.has(species)) {
          team.set(species, {
            ...
            status: "benched",
            hpPct: 100,
            item: itemField || (items?.[species] ?? null),
            condition: null,
          });
        }
        continue;
      ...
      if (
        trimmed.startsWith("|teampreview") ||
        trimmed.startsWith("|clearpoke") ||
        trimmed.startsWith("|start") ||
        trimmed.startsWith("|teamsize|") ||
        trimmed.startsWith("|gametype|") ||
        trimmed.startsWith("|player|") ||
        trimmed.startsWith("|gen|") ||
        trimmed.startsWith("|tier|") ||
        trimmed.startsWith("|rule|") ||
        trimmed.startsWith("|rated")
      ) {
        continue;
      }
      ...
      if (trimmed.startsWith("|switch|") || trimmed.startsWith("|drag|")) {
        ...
        const label = trimmed.startsWith("|drag|")
          ? "was dragged out"
          : "was sent out";
        pushEvent({
          type: "switch",
          text: `${clean(who)} ${label} (${details.replace(/, L\d+.*$/, "").trim()})`,
        });
        if (isP1) p1Active = species;
        else p2Active = species;
        continue;
      }
      ...
      if (trimmed.startsWith("|detailschange|")) {
        ...
        if (member && newSpecies !== member.species) {
          ...
          pushEvent({
            type: "switch",
            text: `${clean(who)} transformed into ${newSpecies}`,
          });
        }
        continue;
      }
      ...
      if (trimmed.startsWith("|replace|")) {
        ...
        nickToSpecies.set(nickname, newSpecies);
        if (isP1) p1Active = newSpecies;
        else p2Active = newSpecies;
        pushEvent({
          type: "switch",
          text: `${clean(who)}'s Illusion wore off`,
        });
        continue;
      }
      ...
      if (trimmed.startsWith("|-heal|")) {
        ...
        pushEvent({
          type: "heal",
          text: `${clean(who)} healed to ${hpRaw}${suffix}`,
        });
        continue;
      }
      ...
      if (trimmed.startsWith("|-curestatus|")) {
        ...
        const msg =
          status === "slp"
            ? `${clean(who)} woke up`
            : status === "brn"
              ? `${clean(who)}'s burn was healed`
              : status === "psn" || status === "tox"
                ? `${clean(who)} was cured of poison`
                : status === "par"
                  ? `${clean(who)} is no longer paralyzed`
                  : status === "frz"
                    ? `${clean(who)} thawed out`
      ...
      if (trimmed.startsWith("|-cureteam|")) {
        ...
        pushEvent({
          type: "status",
          text: `${clean(who)}'s team was cured of status`,
        });
        continue;
      }
      ...
      if (trimmed.startsWith("|-setboost|")) {
        ...
        pushEvent({
          type: "boost",
          text: `${clean(who)}'s ${label} was set to ${amt}`,
        });
        continue;
      }
      ...
      if (
        ...
        trimmed.startsWith("|-clearallboost") ||
        trimmed.startsWith("|-invertboost|") ||
        trimmed.startsWith("|-copyboost|") ||
        trimmed.startsWith("|-swapboost|") ||
        ...
        if (trimmed.startsWith("|-clearallboost")) {
          ...
          if (who)
            pushEvent({
              type: "boost",
              text: `${clean(who)}'s stat changes were reset`,
            });
        }
        continue;
      }
      ...
      if (trimmed.startsWith("|-supereffective|")) {
        ...
        pushEvent({
          type: "supereffective",
          text: `Super effective on ${clean(who)}!`,
        });
        continue;
      }
      // |-resisted|
      ...
      if (trimmed.startsWith("|-start|")) {
        ...
        if (effect === "typechange") {
          ...
          if (typeTag)
            pushEvent({
              type: "volatile",
              text: `${clean(who)}'s type changed to ${typeTag.replace(/^\[from\] /, "")}`,
            });
        } else {
          pushEvent({
            type: "volatile",
            text: `${clean(who)} became affected by ${effectClean}`,
          });
        }
        continue;
      }
      ...
      if (trimmed.startsWith("|-end|")) {
        ...
        pushEvent({
          type: "volatile",
          text: `${clean(who)} is no longer affected by ${effectClean}`,
        });
        continue;
      }
      ...
      if (trimmed.startsWith("|-ability|")) {
        ...
        if (ability)
          pushEvent({
            type: "info",
            text: `${clean(who)}'s ${ability} activated`,
          });
        continue;
      }
      ...
      if (trimmed.startsWith("|-singlemove|")) {
        ...
        const move = parts[3] ?? "";
        if (move)
          pushEvent({ type: "info", text: `${clean(who)} is using ${move}` });
        continue;
      }
      ...
      if (trimmed.startsWith("|-hitcount|")) {
        ...
        const count = parts[3] ?? "";
        if (count && count !== "1")
          pushEvent({ type: "info", text: `Hit ${count} times!` });
        continue;
      }
      ...
      if (trimmed.startsWith("|-weather|") || trimmed.startsWith("|weather|")) {
        ...
        if (w === "none" || w === "") {
          field.weather = null;
          if (!isUpkeep)
            pushEvent({ type: "info", text: "The weather cleared" });
        } else {
          field.weather = w;
          if (!isUpkeep)
            pushEvent({
              type: "info",
              text: `${w} started${from ? ` (${from.replace(/^\[from\] /, "")})` : ""}`,
            });
        }
        continue;
      }
      ...
      if (
        trimmed.startsWith("|-sidestart|") ||
        trimmed.startsWith("|-sideend|")
      ) {
        const side = parts[2] ?? "";
        const condition = (parts[3] ?? "").replace(
          /^(move|ability|item): /,
          "",
        );
        ...
        if (isStart) {
          ...
          pushEvent({
            type: "info",
            text: `${sideLabel}'s ${condition} ended`,
          });
        }
        continue;
      }
      ...
      if (trimmed.startsWith("|-fieldactivate|")) {
        ...
        if (effect)
          pushEvent({ type: "info", text: effect.replace(/^(move): /, "") });
        continue;
      }
      ...
      if (trimmed.startsWith("|-activate|")) {
        const effect = parts[2] ?? "";
        if (effect)
          pushEvent({ type: "info", text: effect.replace(/^\w+: /, "") });
        continue;
      }
      ...
      if (trimmed.startsWith("|-transform|")) {
        ...
        pushEvent({
          type: "volatile",
          text: `${clean(who)} transformed into ${species}`,
        });
        continue;
      }
      ...
      if (
        trimmed.startsWith("|-mega|") ||
        trimmed.startsWith("|-primal|") ||
        trimmed.startsWith("|-burst|")
      ) {
        const who = parts[2] ?? "";
        pushEvent({
          type: "switch",
          text: `${clean(who)} underwent a forme change`,
        });
        continue;
      }
      ...
      if (trimmed.startsWith("|-zbroken|")) {
        ...
        pushEvent({
          type: "info",
          text: `Z-Move broke through Protect on ${clean(who)}`,
        });
        continue;
      }

      // |-combine| / |-waiting| — skip
      if (trimmed.startsWith("|-combine") || trimmed.startsWith("|-waiting"))
        continue;
