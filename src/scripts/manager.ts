...
function xpBarHtml(humon: Humon): string {
  const pct = Math.min(
    100,
    Math.floor(((humon.xp % XP_PER_LEVEL) / XP_PER_LEVEL) * 100),
  );
...
function battleMemberHtml(m: {
  ...
  const itemHtml = m.item
    ? `<span class="mgr-bm-item">${esc(m.item)}</span>`
    : "";
  ...
  const condColor: Record<string, string> = {
    ...
    psn: "#c040c0",
    tox: "#c040c0",
    par: "var(--ceefax-yellow)",
    slp: "var(--ceefax-cyan)",
    frz: "var(--ceefax-blue)",
  };
  const condHtml = m.condition
    ? `<span class="mgr-bm-cond" style="color:${condColor[m.condition] ?? "var(--ceefax-white)"}">${condLabel[m.condition] ?? m.condition.toUpperCase()}</span>`
    : "";
  ...
  const typesHtml = types
    ? types
        .map(
          (t) =>
            `<span class="mgr-type" data-type="${esc(t)}">${esc(t)}</span>`,
        )
        .join("")
    : "";
  const abilityHtml = ability
    ? `<span class="mgr-bm-ability">${esc(ability)}</span>`
    : "";
...
function battleFieldHtml(field: {
  weather: string | null;
  terrain: string | null;
  room: string | null;
}): string {
  ...
  if (field.weather)
    effects.push(`<span class="mgr-bf-weather">☁ ${esc(field.weather)}</span>`);
  if (field.terrain)
    effects.push(`<span class="mgr-bf-terrain">≋ ${esc(field.terrain)}</span>`);
  if (field.room)
    effects.push(`<span class="mgr-bf-room">⊞ ${esc(field.room)}</span>`);
  if (effects.length === 0)
    return '<span class="mgr-bf-none">NO EFFECTS</span>';
  return effects.join("");
}
...
function battleViewerHtml(humon: Humon): string {
  ...
  if (battleViewTurns !== turns) {
    ...
    battleViewTurn = 0;
    console.log(
      "[battle log] raw lines:",
      turns.map((t) => ({ turn: t.turn, raw: t.raw })),
    );
  }
  ...
  const p1ActiveCondHtml = p1Active?.condition
    ? activeConditionHtml(p1Active.condition)
    : "";
  const p2ActiveCondHtml = p2Active?.condition
    ? activeConditionHtml(p2Active.condition)
    : "";
  const resultBar =
    cur === totalTurns - 1
  ...
  const cells = sprite.map
    .flatMap((row) =>
      [...row].map((char) =>
        char === "."
          ? "<span></span>"
          : `<span style="background:${sprite.palette[char]}"></span>`,
      ),
    )
    .join("");
...
function monHtml(name: string): string {
  ...
  const typeHtml = types
    ? types
        .map(
          (type) =>
            `<span class="mgr-type" data-type="${esc(type)}">${esc(type)}</span>`,
        )
        .join("")
    : "";
...
function triggerSpriteAnimations(): void {
  ...
  for (const ev of turn.events) {
    if (
      ev.type === "move" ||
      ev.type === "crit" ||
      ev.type === "supereffective"
    ) {
      const m = ev.text.match(/on (.+?)!/);
      if (m) {
        const targetName = m[1];
        for (const p of turn.p2 ?? []) {
          if (p.nickname === targetName || p.species === targetName)
            hitSides.add("p2");
        }
        for (const p of turn.p1 ?? []) {
          if (p.nickname === targetName || p.species === targetName)
            hitSides.add("p1");
        }
      }
    ...
    if (ev.type === "heal") {
      ...
      if (m) {
        const name = m[1];
        for (const p of turn.p1 ?? []) {
          if (p.nickname === name || p.species === name) healSides.add("p1");
        }
        for (const p of turn.p2 ?? []) {
          if (p.nickname === name || p.species === name) healSides.add("p2");
        }
      }
    }
  }
  for (const side of hitSides) {
    ...
    if (el) {
      el.classList.remove("mgr-bt-sprite-hit");
      void (el as HTMLElement).offsetWidth;
      el.classList.add("mgr-bt-sprite-hit");
    }
  }
  for (const side of healSides) {
    const el = document.querySelector(`.mgr-bt-sprite[data-bt-side="${side}"]`);
    if (el) {
      el.classList.remove("mgr-bt-sprite-heal");
      void (el as HTMLElement).offsetWidth;
      el.classList.add("mgr-bt-sprite-heal");
    }
  }
}
