  ...
  const cards = PLAYERS.map((player) => {
    ...
    return `
			<div class="gym-card">
				...
				<div class="gym-team-chips">${currentTeam(player)
          .map((name) => monChipHtml(name))
          .join("")}</div>
  ...
  const natureRow =
    mon.nature || mon.teraType
      ? `<div class="gym-import-row"><span class="gym-sub">NATURE:</span> ${esc((mon.nature ?? "-").toUpperCase())}${
          mon.teraType
            ? ` <span class="gym-chip">TERA ${esc(mon.teraType.toUpperCase())}</span>`
            : ""
        }</div>`
      : "";
  return `
...
function draftPool(): string[] {
  if (customFoeTeam) {
    const banned = new Set(
      customFoeTeam.flatMap((mon) => [mon.species, speciesToKey(mon.species)]),
    );
...
function foeBannedList(): NameSpecies[] {
  if (customFoeTeam)
    return customFoeTeam.map((mon) => ({
      label: mon.label,
      species: mon.species,
    }));
  return currentTeamOfOpponent().map((name) => ({
    label: name,
    species: name,
  }));
}
...
function draftHtml(): string {
  ...
  const bannedChips = foeBannedList()
    .map(
      ({ label, species }) =>
        `<span class="gym-banned">${bannedChipHtml(label, species)}</span>`,
    )
    .join("");
  if (ownImportTeam) {
    return `
...
function battleHtml(): string {
  if (!snapshot)
    return '<p class="gym-note">CONNECTING TO BATTLE SIMULATOR...</p>';
...
function foeTeamEntries(): OwnMon[] {
  return foeNameList().map(({ label, species }) => {
    ...
    const matches =
      snapshot?.p2Mons.filter(
        (m) => m.name === label || (species && m.species === species),
      ) ?? [];
    const mon =
      matches.find((m) => m.activeSlot !== null) ?? matches[matches.length - 1];
    return {
      name: label,
...
function benchChipsHtml(bench: OwnMon[]): string {
  if (bench.length === 0)
    return `<span class="gym-bench-empty">NO BENCH</span>`;
  return bench
    .map(
      (mon) => `
...
function benchHtml(side: "p1" | "p2"): string {
  ...
  const preStart =
    snapshot.turn === 0 &&
    snapshot.p1ActiveIdents.every((ident) => ident === null) &&
    snapshot.p2ActiveIdents.every((ident) => ident === null);
...
function activeMons(side: "p1" | "p2"): (GymMonView | null)[] {
  ...
  const idents =
    side === "p1" ? snapshot.p1ActiveIdents : snapshot.p2ActiveIdents;
  ...
  return [0, 1].map((slot) => {
    ...
    if (!ident) {
      ...
      return (
        pool.find((mon) => !mon.fainted && mon.activeSlot === slot) ?? null
      );
    }
...
function sideConditionsHtml(side: "p1" | "p2"): string {
  if (!snapshot) return "";
  const conds =
    side === "p1" ? snapshot.p1SideConditions : snapshot.p2SideConditions;
...
function showDescTip(target: Element): void {
  ...
  descTarget = target;
  tip.innerHTML = `<span class="gym-desc-title">${esc(desc.title)}</span><span class="gym-desc-text">${esc(desc.text)}</span>${key.startsWith("move:") ? moveMatchupHtml(key.slice("move:".length)) : ""}`;
  placeDescTip();
}
...
function moveMatchupHtml(moveName: string): string {
  ...
  const segs = activeMons("p2")
    ...
    .map((mon) => {
      ...
      if (!eff) return null;
      const cls = eff.startsWith("NO EFFECT")
        ? " gym-eff-none"
        : eff.includes("SUPER")
          ? " gym-eff-super"
          : "";
    ...
    .filter(Boolean);
  return segs.length
    ? `<span class="gym-desc-match">${segs.join('<span class="gym-eff-sep"> &middot; </span>')}</span>`
    : "";
}
...
function onDescInspect(event: Event): void {
  const target =
    event.target instanceof Element
      ? event.target.closest("[data-gym-desc]")
      : null;
...
function onDescLeave(event: Event): void {
  ...
  const related =
    (event as MouseEvent).relatedTarget ?? (event as FocusEvent).relatedTarget;
  const to =
    related instanceof Element ? related.closest("[data-gym-desc]") : null;
  if (to === from) return;
  hideDescTip();
}

function logHtml(): string {
  if (!snapshot || snapshot.log.length === 0)
    return '<p class="gym-note">BATTLE STARTING...</p>';
  ...
  return tail
    .map((line) => {
      ...
      else if (/FAINTED|YOU WIN|WINS!|TIE GAME/.test(line))
        cls = " gym-log-big";
...
function movePromptHtml(): string {
  ...
  const panels = prompt.actives
    .map((active, i) => slotPanelHtml(active, i))
    .join("");
...
function slotPanelHtml(active: GymActivePromptData, i: number): string {
  ...
  if (pend)
    return `<div class="gym-slot-panel is-done">${head}<div class="gym-slot-body"></div></div>`;
  if (awaitingTarget && awaitingTarget.slotIndex === i) {
    ...
    const foeActive = mp
      ? activeMons("p2").filter((m): m is GymMonView => !!m)
      : [];
    const options =
      move && mp
        ? targetOptionsFor(move.target, move.name, mp.actives, i, foeActive)
        : [];
    const list =
  ...
  const moves = active.moves
    .map((move) => {
      ...
      const pp =
        move.maxpp > 0 ? ` <small>${move.pp}/${move.maxpp}</small>` : "";
...
function pendingLabel(pend: PendingAction): string {
  if (pend.type === "switch")
    return `SWITCH &rarr; ${esc(pokemonAt(pend.pos)?.toUpperCase() ?? "?")}`;
  const move =
    prompt?.kind === "move"
      ? prompt.actives[pend.slotIndex]?.moves.find(
          (m) => m.slot === pend.moveSlot,
        )
      : undefined;
...
function targetLabel(pend: PendingMove): string {
  ...
  const move = prompt.actives[pend.slotIndex]?.moves.find(
    (m) => m.slot === pend.moveSlot,
  );
  const foeActive = activeMons("p2").filter((m): m is GymMonView => !!m);
  const options = targetOptionsFor(
    move?.target ?? "normal",
    move?.name ?? null,
    prompt.actives,
    pend.slotIndex,
  ...
  return (
    options.find((o) => o.loc === loc)?.label ?? pend.locStr
  ).toUpperCase();
}
...
function forcedSwitchHtml(): string {
  ...
  const slots = prompt.slots
    ...
    .map(({ index }) => {
      ...
      return `
            ...
            pickedPos || pass
              ? ""
              : `<div class="gym-slot-body"><div class="gym-switches">${bench
                  .map(
                    (mon) =>
                      `<button class="mgr-btn mgr-btn-sm" data-gym="forced-pick" data-slot="${index}" data-pos="${mon.pos}">${esc(mon.name.toUpperCase())} ${mon.hpPct}%</button>`,
                  )
                  .join("")}</div></div>`
          }
				</div>`;
  ...
  const allResolved = prompt.slots
    .filter(({ must }) => must)
    .every(
      ({ index }) =>
        forcedPicks[index] !== undefined || switchableBench(index).length === 0,
    );
  return `
		<div class="gym-prompt">
...
function ownTeam(): OwnMon[] {
  ...
  const slots =
    ownImportTeam ?? pickedTeam.map((label) => ({ label, species: undefined }));
  return slots.map(({ label, species }, i) => {
    const mon = snapshot?.p1Mons.find(
      (m) => m.name === label || (species && m.species === species),
    );
    return {
      name: label,
...
function bannedChipHtml(label: string, species?: string): string {
  const sprite = POKEMON_SPRITES[label]
    ? `<img src="${POKEMON_SPRITES[label]}" alt="" loading="lazy" />`
    : monImgHtml(species ?? label);
...
function monImgHtml(name: string, species?: string): string {
  if (POKEMON_SPRITES[name])
    return `<img src="${POKEMON_SPRITES[name]}" alt="" loading="lazy" />`;
  if (species && POKEMON_SPRITES[species])
    return `<img src="${POKEMON_SPRITES[species]}" alt="" loading="lazy" />`;
  const resolved = monSprite(species || name);
  if (resolved.url)
    return `<img src="${esc(resolved.url)}" alt="${esc(name)}" loading="lazy" />`;
...
function typesHtml(
  ...
  const types =
    currentTypes ?? POKEMON_TYPES[name] ?? dexTypes(species || name);
  return types
    .map(
      (t) =>
        `<span class="mgr-type" data-type="${esc(t)}">${esc(String(t).toUpperCase())}</span>`,
    )
    .join("");
}

function pixelSpriteHtml(
  map: string[],
  ...
  const cells = map
    .flatMap((row) =>
      [...row].map((char) =>
        char === "."
          ? "<span></span>"
          : `<span style="background:${palette[char]}"></span>`,
      ),
    )
    .join("");
...
function handle(action: string, el: HTMLElement): void {
  switch (action) {
    ...
    case "start-battle":
      if (
        humanReady() &&
        (customFoeTeam || (opponentNumber !== null && foeTeam.length === 4))
      )
        startBattle();
      break;
    case "team-order": {
    ...
    case "team-confirm":
      if (teamOrder.length === 2) {
        ...
        battle?.submitTeamOrder(
          [...teamOrder, ...rest].map((index) => index + 1),
        );
        teamOrder = [];
        prompt = null;
        render();
    ...
    case "choose-move": {
      ...
      const move = prompt.actives[slotIndex]?.moves.find(
        (m) => m.slot === moveSlot,
      );
...
function startBattle(): void {
  ...
  const engine = new GymBattle({
    ...
    nameMap: Object.fromEntries(
      [...(ownImportTeam ?? []), ...(customFoeTeam ?? [])].map((mon) => [
        mon.species,
        mon.label,
      ]),
    ),
