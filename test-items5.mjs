...
const ai1 = new RandomPlayerAI(ps.p1, { seed: prng }),
  ai2 = new RandomPlayerAI(ps.p2, { seed: prng });
void ai1.start();
void ai2.start();
// Try gen9ou with items
void ps.omniscient.write(
  ">start " +
    JSON.stringify({ formatid: "gen9ou", seed: prng.getSeed() }) +
    "\n>player p1 " +
    JSON.stringify({
      name: "HUMON",
      team: buildTeam(["Pikachu", "Charizard", "Blastoise"]),
    }) +
    "\n>player p2 " +
    JSON.stringify({
      name: "GYM",
      team: buildTeam(["Garchomp", "Tyranitar", "Gengar"]),
    }),
);
...
for (const chunk of raw) {
  for (const line of chunk.split("\n")) {
    const t = line.trim();
    if (t.includes("poke|")) console.log(t);
  }
}
