...
- **`src/data/humon-skins.ts`**: post-victory HUMON skins — `HUMON_SKINS` (id, name, sprite), `skinFor(id)` falls back to `default`; `CHAMPION_SPRITE` is a gold crown + gold-outfit HUMON; the JOAK/DEVIL/COP skins reuse the hidden-humon sprites (`skinUnlockKey` gates them by catch). Stored in `GameState.cosmetic.skin` via `setHumonSkin` (champion skins keep the version at 4).
