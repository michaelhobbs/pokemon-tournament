...
- **Hidden humons** = secret pages, each caught with its eponymous ball bought at the POKESHOP on 810 and thrown on the humon's hidden page: Joak (123, **JOAKBALL** ¥500), Devilmon (666, **DEVILBALL** ¥750), Copmon (999, **COPBALL** ¥750), Glitchmon (404, **GLITCHBALL** ¥600 — repurposed error page, "ONLY GLITCHMON EXISTS"), Bondmon (007, **BONDBALL** ¥800), Angelmon (777, **ANGELBALL** ¥700), Sexmon (069, **SEXBALL** ¥750) and Dbzmon (329, **DBZBALL** ¥850, "POWER LEVEL OVER 9000"). Each hidden humon joins the squad with a themed full team of 6 (all non-shiny, straight from the 67-species tournament pool) — no species shared between any two teams. Teams: Joak top-tier (Dragonite, Garchomp, Salamence, Metagross, Tyranitar, Hydreigon), Devilmon all-fire (Hisuian Typhlosion, Torkoal, Charizard, Blaziken, Chandelure, Talonflame), Copmon the law (Alolan Ninetales, Cloyster, Mamoswine, Lapras + Arcanine, Lucario), Glitchmon the corrupted (Porygon2, Rotom (W), Aegislash, Gengar, Sableye, Galarian Weezing), Bondmon the tuxedo spy (Greninja, Murkrow, Meowstic (M), Hisuian Zoroark, Kingdra, Kangaskhan), Angelmon the watcher (Sylveon, Togekiss, Clefable, Gardevoir, Milotic, Pelipper), Sexmon, "69. NICE." (Blissey, Whimsicott, Alolan Muk, Blastoise, Politoed, Raichu) and Dbzmon (Electabuzz, Hariyama, Hitmontop, Scizor, Shuckle, Slaking). The SEXMON/DBZMON copy runs on crude PokéCeption-meme innuendo, not explicit content.
...
**`src/data/hidden-humons.ts`**: specs + sprites for the 8 hidden humons (new 16×16 PixelArt — GLITCH/BOND/ANGEL/SEX/DBZ plus the existing Devilmon, Copmon, and Joak via `JOAk_SPRITE`), each with unlock cost + required visited page + a `team: string[]` of the themed 6 pokémon they arrive with. `BALL_NAMES`/`BALL_SPRITES` map every humon to its eponymous ball.

## New pages (800s block)

**810 MANAGER** (single page, `/manager`) renders everything stacked in one scroll: day/currency/stamina status bar, GO TO SLEEP, victory banner, objective, quick-jump bar (scrolls to each section), full roster (levels, XP, teams, stamina, rare candy), TRAIN form, TRAVEL form + live catch-pool preview, GYM LEADERS boss cards, battle replays, a **POKEPUTER** section (boxed duplicates with SELL / unbox-to-humon controls plus a FROM→MON→TO transfer form for moving pokémon between humons and the box), POKESHOP (8 hidden-humon balls, ¥500–¥850 each — throw on the humon's hidden page to catch it — and **RARE CANDY ¥150 each, unlimited** for instant +50 XP on any humon), a BADGE RACK (10 slots, filled with each defeated leader's badge), and the recent-activity log. After winning (`wonDay` set) a **CUSTOMIZE HUMON** section unlocks: buttons pick a new sprite skin for the header HUMON, with all 8 hidden-humon skins gated behind catching those hidden humons. Shows a gate message if no HUMON is caught.

Plus hidden pages **`123.astro` (PROFESSOR JOAK)**, **`666.astro` (DEVILMON)**, **`999.astro` (COPMON)**, **`404.astro` (PAGE NOT FOUND → GLITCHMON — also serves as the site's 404 page)**, **`007.astro` (BONDMON)**, **`777.astro` (ANGELMON)**, **`069.astro` (SEXMON)** and **`329.astro` (DBZMON)**, each with a ball-throw widget (`data-manager-secret`) fed by balls from the POKESHOP.

## Wiring

- **`navigation.ts`**: `MANAGER_PAGE` (810, hidden) — add to `PAGES`, `ALL_PAGES`; `BONDMON_PAGE`/`SEXMON_PAGE`/`DBZMON_PAGE`/`ANGELMON_PAGE`/`DEVILMON_PAGE`/`COPMON_PAGE` (hidden) plus `NOT_FOUND_PAGE` (404) so typing 007/069/123/329/404/666/777/999 works.

- **`src/data/humon-skins.ts`**: post-victory HUMON skins — `HUMON_SKINS` (id, name, sprite), `skinFor(id)` falls back to `default`; `CHAMPION_SPRITE` is a gold crown + gold-outfit HUMON; the 8 hidden-humon skins reuse their sprites (`skinUnlockKey` gates them by catch). Stored in `GameState.cosmetic.skin` via `setHumonSkin` (skins keep the version at 4).
- **`src/scripts/manager.ts`**: global init (included once, like `ceefax-nav.ts`); on every `astro:page-load` loads state, marks the current page as visited (for unlock conditions), and re-renders any `data-manager-feature` containers. A small `ManagerApp.astro` component injects it per page with a feature key (`all`). Forms use scoped select keys (`train-humon`, `travel-humon`/`travel-town`, `gym-humon`) safe to co-exist on the single page. After every action it dispatches a `mgr:say` event (newest log line) and a `mgr:update` event; the header HUMON shows the newest log line in a 5-second transient bubble (replaced on the next action; `is-talking-say`), paints a chest-medal pin per earned gym badge (colored squares in defeat order), and displays the customized skin from `loadState()`.
...
Max stamina 100 +20/level · train 20 stamina/40 XP · travel 40 stamina/25 XP · gym 50 stamina/60 XP · sleep restores all · gym win ¥150–300 · travel/train ¥20–80 · JOAKBALL ¥500 · GLITCHBALL ¥600 · DEVILBALL/COPBALL/SEXBALL ¥750 · ANGELBALL ¥700 · BONDBALL ¥800 · DBZBALL ¥850 · RARE CANDY ¥150 (unlimited) · all actions immediate.
