...
- `DEFAULT_STATE`, `loadState()`, `saveState()` under key `pkm:manager:v2` (pattern from `humon.ts`, try/catch wrappers). v1 saves are migrated on load (fresh stamina, pending timers dropped); v2 saves migrate to `CaughtMon[]` (all non-shiny).
